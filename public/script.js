...
async function placeBet() {
  ...
  try {
    const res = await fetch("/api/v1/bet", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
async function cashout() {
  ...
  try {
    const res = await fetch("/api/v1/cashout", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
async function fetchWallet() {
  ...
  try {
    const res = await fetch(`/api/v1/wallet/${playerId}`);
    const data = await res.json();
    const w = data.wallet;
    console.log(w)
...
async function fetchHistory() {
  try {
    const res = await fetch(`/api/v1/round-history?limit=5`);
    const data = await res.json();
